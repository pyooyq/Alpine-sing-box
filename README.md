...
菜单支持添加落地、增加入站（代理服务器）、查看节点和落地摘要、管理落地、修改入站设置、TCP/UDP 转发管理、启动/停止/重启 sing-box、查看日志、卸载。
...
1. 使用“添加落地”，直接粘贴 `ss://`、`vless://`、`http://`、`https://`、`socks5://`（支持带/不带密码）或其 base64 内容；脚本识别协议后列出现有入站，由你按序号选择要绑定到该落地的入站。若当前没有任何入站，则自动创建一个绑定该落地的新入站。
2. 进入“管理落地”会按序号列出所有落地及其绑定的入站；选择某个落地后可删除它，或把它改绑到另一个入站。
...
4. 想改某个入站的端口 / SNI / 密码 / 绑定落地，或删除该入站，选择“修改入站设置”，按序号选中入站后操作。
5. 客户端使用脚本生成的对应协议链接连接当前 VPS；服务端会按入站用户路由到绑定的 outbound。
