...
while [ $# -gt 0 ]; do
    case "$1" in
        -port)
            ...
            [[ "$2" =~ ^[0-9]+$ ]] || { red "-port 需要数字端口"; exit 1; }
            [ "$2" -ge 1 ] && [ "$2" -le 65535 ] || { red "-port 端口范围需在 1-65535"; exit 1; }
            vless_port="$2"
            shift 2
            ;;
        -install)
...
[ -n "$vless_port" ] || vless_port=$(shuf -i 1000-65000 -n 1)
export vless_port
...
vless://${uuid}@${server_ip}:${vless_port}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${reality_domain}&fp=firefox&pbk=${public_key}&type=tcp&headerType=none#${isp}
